...
**Persistence:** SAVE_VERSION=6 (4→5 profile-slot rebuild 2026-08-08; 5→6 over-water grace 2026-08-11 — the version ladder history lives at the constant in `GameData.cs`). `MINIMUM_SUPPORTED_SAVE_VERSION` tracks it pre-1.0, so older saves are refused, not migrated. Bumps queued: **6→7 = prestige-pass Stage 5** (wallet + `StartingPlayerShare` + `HighWaterVictoryValue`, objective counters dropped — Bob ruled it does NOT wait for AI2b-3); then AI2 (AIPerceptionState) and P5 (loss ledger) take their own. ⚠ `SaveLoad.SaveAsync`/`LoadAsync` still have NO callers — save/load is not wired to any UI, which is why bumps have been free.
