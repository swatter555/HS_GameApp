...
_Last reconciled against the codebase: 2026-09-03 (second pass) — **SPRITE MANIFEST AUDITED AGAINST THE CODE (no code change).** All 172 sprite-bearing profiles walked: zero dangling `SpriteManager` constants, zero genuine gaps in the master list (the 22 unlisted names are all pre-rename spellings the pending batch retires — `AR_*` ×14, `GER_*`, `CH_H9`, `CH_Type82`, `MJ_AA`, `SV_AA`, `SV_AN8`, `UK_TornadoGR1` — whose replacements are already listed), and zero rows falsely tagged On-disk. Four profiles point at a PNG that does not exist: the three art-gated German air-mobile sprites, and **`SV_2S5_W`, which exists nowhere under `Assets/Art/Sprites`** despite being ticked on the manifest. ⚠ **`ME_Airbase` is the one unit profile whose art lives OUTSIDE `Art/Sprites/Unit Icons/`** (it is in `Map Icons/`) — it looks like a missing sprite to any check scoped to the unit directory, and the T-4/T-5 suffix sweep is scoped to exactly that directory. ⚠ The audit was a THROWAWAY script, deliberately not committed. **Its durable home is an Editor test** — `AssetDatabase.FindAssets` can assert a declared icon resolves to a real PNG, closing the one gap `IconIntegrityTests` structurally cannot (it proves an icon is declared and well-formed, never that the file exists). That is the `SV_2S5` defect class. Not built — offered to Bob, awaiting his call._
_Prior: 2026-09-03 — **GE AIR-MOBILE BRIGADE, the first RE-5/RE-6 slice (suite GREEN, Bob-run):** three profiles (`INF_AM_GE` · `APC_M113_GE` · `HEL_UH1D_GE`), three WeaponType members, eight `SpriteManager` constants and the `GE_AIRMOBILE_BRIGADE` template — Deployed/Mobile/Embarked, mirroring `US_AIRMOBILE_BRIGADE`. **Now 186 profiles / 183 templates / 215 WeaponType members.** ⚠ **COUNT CORRECTION: the RE-1a stamp's "183 templates" was wrong — it was 182** (profiles were right); verified by counting call sites with and without the change stashed. ⚠ **ART-GATED ON PURPOSE:** all three sprites are still to-draw, so the unit renders the mismatch placeholder until Bob's German drop lands — the same deliberate state `SV_2S5` sat in, and `IconIntegrityTests` cannot catch it by design (it proves an icon is declared and well-formed, never that the PNG exists). `INF_AM_GE` differs from `INF_AB_GE` by exactly ONE trait (`MOUNTAIN_TRAINED` for `AIR_DROPPABLE`), the same split the US AM/AB pair uses, and both halves are pinned because a one-trait difference is what silently collapses into a re-skin. `APC_M113_GE` is the SAME VEHICLE as `APC_M113_US` and is pinned to resolve identically — it exists only because art lives on the profile, so divergence is a bug, not a national variant. The UH-1D is TRANSPORT ONLY (Germany never armed its Hueys; the Bundeswehr gunship is the Bo 105 PAH-1), hence no `HEL_UH1C_GE` to match the US/Saudi pair. ⚠ **NO new bay test was written, deliberately** — the three `EquipmentBaysTests` template audits walk the REAL database, so a new template is swept the moment it is authored. That is the whole reason those audits walk the DB instead of a fixture list; adding a hand-written test per template would re-create the blind spot they exist to close._
_Prior: 2026-09-02 (second pass) — **CONSOLE HYGIENE + THE TEST HOLE BEHIND IT (suite GREEN 651/651, Bob-run):** the whole EditorTest run emits five console messages. Two are Unity's — the `IPrebuildSetup`/`IPostBuildCleanup` pair from `com.unity.test-framework.performance`, a **depth-3 transitive package** nothing in `Assets/` references; they cannot be removed and are not a symptom. One was gratuitous (`SkillBranchExtensions` announcing a successful cache init — deleted; its failure paths already report themselves, and its other `Debug.Log`s are `[Conditional]` opt-in tools that were left alone). ⚠ **The remaining three exposed a real gap: each came from a test NAMED for a warning it never asserted** — `Stamp_NonStrongholdObjective_WarnsButStamps` (checked only the stamp), the legacy map-dimension fallback (only the returned size), and `IsValid_EarlyFinishMultiplierExactlyOne_LoadsButIsInert` (only the bool, while its own comment called the warning the guard). In all three the warning IS the ruled behaviour and nothing refuses, so deleting the `LogWarning` would have left every one green with the guard gone. Now pinned with **`LogAssert.Expect`** — first use in the suite; `UnityEngine.TestRunner` was already on the asmdef — matching a stable `Regex` fragment rather than full prose so the messages stay re-wordable. **Standing rule from this: a test whose name claims a warning must assert the warning; `Debug.LogWarning` in a non-refusing branch is a CONTRACT, not decoration.** ⚠ **AND THE THING TO REMEMBER SO NOBODY RETRIES IT: `LogAssert.Expect` DOES NOT SILENCE THE CONSOLE.** `LogScope` subscribes to `Application.logMessageReceivedThreaded` — an observer that fires after Unity has already written the line — so the warnings still print and still land in the run's `<output>`. The only lever that would hide them (`Debug.unityLogger.logEnabled = false`) also stops the message reaching `logMessageReceived`, which makes the expectation go unmet and FAILS the test. Asserting a warning and hiding it are mutually exclusive; asserted wins. The three lines are correct output from 651 tests exercising three warn-but-allow branches, and they stay._
_Prior: 2026-09-02 — **RE-4 `SECOND_LINE_FORMATION` (roster expansion §5; suite GREEN, Bob-run):** the §13 formation-quality trait layer gains its **first sub-1.0 member** at `Icm(0.9f)` and **China leaves the ICM-1.0 baseline** it shared with the Soviets and the Arabs — the incompletely reformed late-1980s Group Army is priced by ICM rather than by stripping equipment off the census. Applied to **15 of the 16 Chinese profiles**: every one a template names in its DEPLOYED bay. ⚠ **`IFV_TYPE86_CH` is deliberately EXCLUDED** — it exists only as the Mobile-bay ride of the mechanised regiment, and closed-bay doctrine prices the formation ONCE, on the sole/deployed profile; a pin in `WeaponProfileChineseTests` guards that. Resolved: China 0.90, Type 80 0.945 (LRF 1.05 × 0.9), Type 86 1.00. ⚠ **The RE-3a commodity blueprints now take `params WeaponTrait[]`** (new private `Plus()` helper in `WeaponProfileDB`) so a national trait rides ON a shared stat line instead of forking it — the Chinese towed guns keep the Soviet ballistics and carry a different ICM, pinned both ways. `HS_DesignDoc` §7.5.5 + `WeaponTrait_Supplement` §13b amended in step (new T91; the "Soviet/Chinese/Arab stay ICM-1.0" anchor retired in place). ⚠ FOUND IN PASSING: **`SPA_TYPE83_CH` has a profile but no template** — the only Chinese profile nothing fields; booked as RE-6. Tests: 5 Chinese ICM pins corrected, 3 new (the sweep, the China-only tripwire, the blueprint guard). Same day, docs only: the **Huey ownership ruling** — four owned sprite sets (US + Saudi own a transport/gunship pair, NATO generic borrows the US art, Iran owns `IR_UH1`, Germany keeps `GE_UH1D`), and the `XX_XXXX` naming rule confirmed to govern SPRITE names only, never WeaponTypes._
_Prior: 2026-08-29 (second pass) — **RE-1a ROSTER RENAME BATCH (SAVE_VERSION 10; suite GREEN + Khost fine, Bob-run; plan `Planning Docs/Roster Expansion.md` §2):** 20 WeaponType renames / 169 replacements. Both Tornado types now name the NATION each already served — GR.1 is the RAF designation for the IDS, and the two profiles had been authored as British variants with Germany borrowing the spare; designations and art pointers swapped, **no stat line moved**. `SPA_M109_FR`→`SPA_AUF1_FR` (it is an AUF1); `APC_FV432`→`_UK`, `HEL_AH1`→`_US`; the three `WEST` types →`_NATO`; thirteen Chinese types gain `_CH`, two with model corrections (`SPA_TYPE82`→`SPA_TYPE83_CH`, the PLZ-83 — the Type 82 is a 130mm MRL; `HEL_H9`→`HEL_Z9_CH` — "H" is bomber, the aircraft is the Harbin Z-9). DELETED: `TANK_TYPE95` (fictional), and two content-error templates — `UK_AIR_DEFENSE_REGIMENT` (Britain never operated the M163) and `IR_SAM_REGIMENT` (Iran never operated the SA-2) — plus six `FR_Gepard` constants. ⚠ `TRN_AN8_SV`→`TRN_AN12_SV` deliberately DEFERRED (RE-1b): the only renamed type inside shipped content (`khost.oob`), so it rides the next editor re-export. **Editor DB re-import owed.** Now 183 profiles / 183 templates / 212 WeaponType members. RE-2 also re-pointed four profiles at national art that was shipping unused (`US_AirMobile`, `CH_LightArt`, `CH_HeavyArt`, `SV_AA`)._
_Prior: 2026-08-29 — **TOP-DOWN ICONS T-1…T-3 (suite GREEN + Khost loads clean, Bob-run):** unit art is ONE sprite per profile, rotated to facing. `RegimentIconProfile` collapsed six fields to one `Icon`; `RegimentIconType` down to two members (`Directional`/`Directional_Fire` DELETED with the whole variant+firing art path); all 184 declarations converted (89+53+32 → 174 `Single`, 10 `Helo_Animation` unchanged — all verified still on `_Frame0`, the §1 trap); `NormalizeDirection`/`ShouldFlipSprite`/`out bool shouldFlip`/both `flipX = true` writes deleted; NEW `ApplyIconFacing` rotates `unitIcon` ONLY (never the root — six renderers) and skips bases. NEW `GameData.ICON_MOTION_FRAME0_SUFFIX` is the single spelling of the helo flipbook contract, aliased const-from-const by `Prefab_CombatUnitIcon`. NEW `IconIntegrityTests` (8) closes the "no test touches icons at all" gap. ⚠ **T-4/T-5 STILL OWED and land TOGETHER** — the ~250 `SpriteManager` constant renames would break every lookup until Bob's PNGs are renamed, so profiles still point at the pre-pass `_W` constants (which resolve fine, and are now rotated). ⚠ `ICON_HEADING_OFFSET_DEGREES` assumes the art points WEST; one constant to change if not. See `Planning Docs/Top-Down Icons.md`._
_Prior: 2026-08-24 (second pass) — **SUP-1 SUPPLY UNIFICATION (SAVE_VERSION 9; plan/record `Planning Docs/Supply Unification.md`):** ONE supply number per unit — `CombatUnit.StockpileInDays` DELETED; a depot's pool IS its `DaysSupply` (Max = size cap 30/50/80/110, set by `SetDepotSize`, the single sizing authority for ctor + upgrade); airbase 30 unchanged; **fixed-wing Max 0** (no own supply, the airbase pays §11.2.3 — and `CanMove`'s new `Max > 0` guard is what lets a 0-supply aircraft still take move orders). SnapshotMapper's depot force-reset deleted (it was a live bug: stockpile never restored, half-spent depots reloaded FULL). `.oob` unchanged in STRUCTURE; `DaysSupply` carries every unit's number (khost: depots 30/80, Su-17s 0) — the day-old `StockpileInDays` field RESCINDED before the editor built it. Unit panel: one uniform `Supply: cur/max` line. `DepotStockpileTests` → `DepotSupplyTests` (9). Doc amendment §15.1.2a ratified in step. Same day, earlier: A-1 audio fixes (NEW `JsonPolicy.Settings` closes the last local options; briefing absent-vs-corrupt split), D-3 efficiency renames (`EFFICIENCY_MOD_*_OPS`, values unchanged), `.oob` `DaysSupply` ratio→REAL DAYS, `RiverSymmetryVerifier` repaired (read via `JsonPolicy.Content`)._
_Prior: 2026-08-24 — **DOCS REORG, no code change.** The repo root now holds exactly four markdown files (`CLAUDE.md`, this file, `Claude_TODO.md`, `README.md`); all 21 plan/pass/courier documents moved into **`Planning Docs/`** — see the NEW §1.1 for the rule and the reason. Every path reference in this file and in `Claude_TODO.md` was re-pointed in the same commit. NEW focus file `Planning Docs/Implementing Combat.md` opens the audio / combat-animation / air-ops / combat-constants pass; `Claude_TODO.md` carries only its thread-board row._
_Prior: 2026-08-22 (third pass same day) — **THE AD ENGAGEMENT-RANGE FIX** (the ICM pass's "BIG FIND", spun off + closed; plan/record `Planning Docs/todo_adrange.md`): `SpottingService.FindTransitAirDefense` now reads the AD envelope as **`max(ActiveIndirectRange, ActivePrimaryRange)`** (floor, ≤0→2 fallback kept) — per §11.4.4 the envelope IS the profile's authored IR stat (T71 "IR high" idiom; all ~22 AD profiles author one, verified no gaps — ⚠ TRAIT-COMPOSED: Tunguska = base 3 + GUN_MISSILE_COMBO +2 → 5, read RESOLVED values never bare deltas). **SAME-DAY FOLLOW-ON RULING (Bob): the point-defense re-band** — Chaparral/Roland/Crotale/Rapier + HQ-7 were at the flat SAM 6 (an area-defense umbrella on ~6–12 km systems, invisible while everything engaged at 1) → all five re-authored `INDIRECT_RANGE_SHORT` 4; Tunguska ruled to STAY 5 (apex short-range, above Strela-1 4, below Kub 6); Hawk stays 6 (genuine area defense); ratified ladder in §11.8.2d: guns 3 · point-defense 4 · Tunguska/S-125 5 · area SAM 6 · S-300 10 (NATO deliberately has no 10 — no Patriot). Envelope pins added: NATO suite ×5, Soviet ×3 (incl. the trait-composed Tunguska 5), Chinese ×1. Before this, the PR read (no AD profile sets PR; archetype default 1) made **every battery interdict at range 1** — the whole authored ladder was dead data, invisible in play because Khost's AD is short-range. NOT a new ruling — a defect fix against the already-ratified §11.4.4; recorded as NEW DesignDoc §11.8.2d. Khost play-feel: MJ Stinger/AAA envelopes go 1→3 hexes (the intended value). ⚠ AD prestige was tuned against the INTENDED envelopes — do not re-price around this. ⚠ The §11.4.4 "spotting +" term is deliberately NOT decided here — ruled once at M13 `AirThreatService` so overlay + walk agree. `AirDefenseTransitTests`: `Reach()` helper mirrors the new read + 4 new tests (S-300 engages at 10/refuses at 11, ZSU at 3/refuses at 4, ladder differentiation, §11.8.6 one-shot across a wide envelope). ⚠ Suites need a Bob run._
_Prior: 2026-08-22 (later same day) — **THE PRESTIGE PASS** (Bob-approved items 1–12; analysis + implementation record `Planning Docs/Prestige_SovietEconomy_Analysis_2026-08-22.md`): Soviet-menu re-tiers — BTR-70 → 40, BTR-80 → 100 (the BTR line was dead stock above the BMPs), BRDM-2 AT → 105, Su-24 → 300 (BMB type cost — it was a Fencer priced as a Fitter), T-80B → 125 (was a 185 trap beside HD14-15 peers); M60A3 ACR → explicit 250 (`GameData.PRESTIGE_ACR_SQUADRON`). **NEW `CombatUnit.PurchaseCost`** (Σ populated bays, `[JsonIgnore]`) is the ratified §18.3.1 whole-unit price — COMPOSABLE (buy bare, add bays later at profile price) — and is now the **V19 kill-bounty basis** (all three `PrestigeOnKill` sites swapped off the active-profile read; bounty no longer moves with the victim's posture; pinned by `PurchaseCost_SumsPopulatedBays_PostureIndependent`). §18.5.1 upgrade formula ratified: `max(target − current, PRESTIGE_UPGRADE_MIN 20)` × 0.7 w/ Connections (constant in GameData; P4 `RequisitionService` consumes it); §18.5.2: same-price turn-gated successors (T-80BVM, Su-47) deliberate, upgrade at the floor. HS_DesignDoc §18 RECONCILED (18.3's stale pre-2026-06-18 table replaced; WW type + air-tier shortcuts formally retired; 18.6/18.7 marked REP-not-prestige; 18.4.1 re-based) and `Planning Docs/todo_profiles.md` §4.7 carries the ratified pricing rules for P4. Income side untouched — live and grounded (Khost: pool 500 + stipend 20 + 0.05 × held value ≈ ~2,000/battle well-played). ⚠ Suites need a Bob run (V19 basis swap + new test)._
_Prior: 2026-08-22 — **THE ICM PASS** (Bob-ratified rulings 1–11, plan+record `Planning Docs/todo_icm.md`; survey `Planning Docs/WeaponProfile_ICM_Prestige_Survey_2026-08-22.md`): **CLOSED-BAY ICM DOCTRINE** — on a unit's SOLE profile the ICM prices the WHOLE FORMATION; paired (Mobile/Embarked-bay) profiles stay 1.0. Four new `WeaponTrait` members (§13 formation quality): `COMBINED_ARMS_TF` ×1.15 (M1 → 1.53), `NATO_FIRST_LINE` ×1.10 (Leo 2 → 1.47, Leo 1 GE → 1.21, Challenger → 1.33), `AIR_CAVALRY` ×1.21 (M60A3 ACR → 1.40 — its census carries 8 AH-64 + 8 OH-58), `FIRE_DIRECTION_NET` ×1.05 (M109 ×4 + MLRS). **RECON RULING:** all six recon profiles fight as SOFT (five `SetTargetClass(Hard)` overrides DELETED — skirmishers with staying power, SD9 defends) and `RECON_FRAGILE` is DORMANT (BRDM-2 back to ICM 1.0). **Artillery:** US M109 gains SMART_MUNITION (HA8/SA11); PHZ-89 re-tiered Gen3→Gen1 (175), Type 82 Gen3→Gen2 (190); ART_HEAVY_CH IR 6→5; MJ mortar/light-arty gained their MISSING prestige (90 — V19 kill-bounty was ZERO for them); five SP-gun templates reclassified ART→SPA (behavior-identical, drift); `US_ARMOR_BRIGADE` Trained→Experienced; the ~156 vestigial `// Set the ICM` comments stripped from CombatUnitDB. HS_DesignDoc §7.4.1.2/§7.5.5/§7A.5 + WeaponTrait_Supplement (T85a dormant, new §13b/T87–T90) amended in step. ⚠ FOUND + SPUN OFF (own session, chip pending): **the §11.8 AD engagement-range defect** — every SAM/AAA profile authors its envelope in IR (S-300 10 … ZSU 3) but `FindTransitAirDefense` reads `ActivePrimaryRange` (=1 on all AD), so every battery interdicts aircraft at range 1 and the authored ladder is dead data. ⚠ Suites need a Bob run (WeaponProfileNato/Soviet/Class tests updated + new FormationQuality pin)._
_Prior: 2026-08-20 — **C7 FRACTIONAL OBJECTIVE GATE + V19** (editor's ask, Bob-ratified; **SAVE_VERSION 8**): the §17.8 gate is now held ≥ ceil(total × `missionObjectiveFraction`) — new manifest float in (0,1], default 1.0 = the old all-of-them rule, mirrored into `ScenarioData` (the stamped flags ride the embedded map; the fraction has no other carrier). ONE predicate for all three gate sites: `HexMapUtil.CountMissionObjectives` (counts, fail-open (0,0) — REPLACES the deleted bool `AllMissionObjectivesHeld`) → `BattleManager.MissionObjectiveGateMet` + `RequiredObjectiveCount` (round-to-4-decimals-then-ceil kills the 0.3f×10→4 float trap, clamp [1,total]). Battle-start diagnostic logs held/total/required + warns on the gate/ladder collision (min gate-met share ≥ decisive cut → middle rungs decorative — the re-priced-Khost defect). V19: `GameData.PRESTIGE_KILL_FRACTION` (0.5) replaces three `cost / 2` copies, away-from-zero rounding, still reported-not-credited. No migration arm (pre-1.0 rule)._
_Prior: 2026-08-19 — **THE THEME-ART PASS** (Bob authored full EU + CH art sets; compile + suites + Khost regression green): map icons are per-theme for ALL THREE themes — `HexGridRenderer.CreateMapIcon` is a 9-arm (theme × icon type) switch, the sprite resolves BEFORE Instantiate with a debounced warn-and-skip for both a missing arm AND an atlas-packing miss; SpriteManager +6 constants (EU_/CH_ × Sprawl/Fort/Airbase); `TextureArrayBuilder`'s China prefix fixed `CN`→`CH` (matches the codebase-wide China prefix and the authored `CH_*.png` tiles); all three `TerrainArray_<Theme>.asset` are baked (~289 MB each, ALL gitignored — §3.5c rebuild-after-clone now covers three arrays). ⚠ Two truths pinned in comments: `BASE_AIRBASE.IconProfile`'s `ME_Airbase` is UNREAD at render (`GameIconRenderer.GetSpriteNameForUnit` short-circuits AIRB to the theme-agnostic `AirbaseStack_N` badges), and fort facility units DO NOT EXIST (forts are tile infrastructure; themed fort art is the map icon). City icons/nameplates/terrain portraits were already theme-wired and needed zero code. EU/CH in-play verification is ⚑-gated on the first non-ME scenario export._
_Prior: 2026-08-17 — **THE PRESTIGE/VICTORY PASS, Stages 1–2** (Bob-ratified via the editor-side change request V1–V16 + its two reply rounds; plan `Planning Docs/todo_prestige.md`; GATES 1–2 both GREEN incl. Khost played): NEW `VictoryLedger` (derived, recomputed once per turn + at battle start, NEVER accumulated) + `PrestigeWallet` (`AddPrestige` finally credits the spendable balance; `SpendPrestige` → bool atomic) + `EventManager.OnPrestigeChanged`; `ScenarioManifest` → parameterless ctor (the 16-param `[JsonConstructor]` is DELETED — it silently defaulted any property lacking a ctor param) + 8 scoring/economy keys (§7.2; two-state thresholds, `requiredResult` by NAME — **`BattleResult` is rename-frozen**; Draw = the defensive scenario, C5); **capture stickiness now DERIVES from `HexTile.IsStronghold`** (cities/fort/airbase/port — `isObjective` is GAMEPLAY-DEAD, a UI marker with exactly two readers until the V15 rip); `TerritoryService` renamed to `StrongholdCapture`/`CapturedStrongholds`; `RegionGraph` → `StrongholdCount` + ungated `VictoryValue`; **the all-objectives instant win is RETIRED** (interim: NO early end exists — battles run to the turn limit until the Stage 4 share-based rule); SV control-flag arm added; map icons warn-and-skip on missing theme art. **THE PASS CLOSED THE SAME DAY — Stages 3–5 (all gates green incl. play):** §18.2 per-turn INCOME is live in the player Upkeep (`BattleManager.ComputeIncome`: stipend + rate × held value + high-water progress bonus — the capture award and the objective counters are DELETED; `ReportStrongholdTaken/Lost` print ledger roll-ups instead); `CompleteBattle` GRADES for real (V9 mirrored ladder around the persisted `StartingPlayerShare`, C1 no-scoring guard, arithmetic logged) with **the C6 MISSION-OBJECTIVE GATE applied last** — manifest `missionObjectives` [{x,y,label?}] are CLEARED-then-STAMPED onto `hex.IsObjective` by `MapLoader.ApplyMissionObjectiveStamp` at scenario load (authored flag value DEAD; out-of-bounds objective REFUSES the load, non-stronghold warns), `HexMapUtil.AllMissionObjectivesHeld` is the gate, and an unmet gate caps the grade one rung below `requiredResult`; V10: share-based nothing-further-to-gain auto-end at TurnBoundary + voluntary early finish via `BattleManager.OnEndScenarioButton` (⚠ NO BUTTON WIRED YET — Bob, Inspector; gate = scoring declared ∧ share ≥ minor ∧ objectives held; bonus = unusedTurns × steady income × multiplier, computed live); **SAVE_VERSION 7** (wallet wiring + `startingPlayerShare` + `highWaterVictoryValue` + the 8 manifest knob mirrors into `ScenarioData` via `CaptureScenarioState`/`RestoreScenarioState` — prestige-pass slice ONLY, full battle-state sync still belongs to the unbuilt save-wiring feature; counter fields dropped; no migration arm). HS_DesignDoc amended in step (§4.7.2, §6.13.8, §17 rework + NEW §17.8/§17.9, §18.2 income model). Both khost manifests carry placeholder scoring + the 12 legacy objective hexes — Khost runs fully scored (1550 value / 36 hexes) on placeholder numbers._
_Prior: 2026-08-13 — **THE CENSUS FIX PASS** (doctrine v2, Bob-ratified; plan `Planning Docs/todo_census.md`, findings `UnitDB_CensusAudit_2026-08-13.md`): every carrier census stripped to own-platform-count (rule 2), lift censuses emptied (rule 4 — Mi-8T, An-12, UH-60), organic tank battalions moved onto the mech BASES (SV +40 T-62A on INF_REG_SV, US/GE/UK +28, NL/BE/DK +32/28/24, IQ/IR +31, CH +40, FR +40 AMX-30), US/GE/UK bases re-cut to battle-group scale (§1a amended — one counter = one maneuver formation; France = division-as-counter, 80 tanks kept), the Iraqi 209-tank copy-paste bug deleted, UK token bugs fixed (recon `SPSAM_RAPIER_UK`→`RCN_FV105_UK`; `MANPAD_RAPIER`/UK Stinger→new census-only `MANPAD_JAVELIN`; MANPAD_RAPIER now census-unreferenced), `INF_REG_NL/BE/DK`→**`INF_MECH_NL/BE/DK`** (renamed while content-free; scheme: INF_LEG_*/INF_MECH_*, shipped names grandfathered), and `CensusIntegrityTests` grown 2→6 (carrier-clean · lift-empty · truck-empty · classification coherence; the name-based CensusExempt allow-list REPLACED by rules). Zero gameplay-behaviour change. See §9 vocabulary, §10.7.9 in the design doc._
...
Assets/StreamingAssets/   STREAMED shipped content (§7.1): Audio/ (ambient, briefings, music — NOT SFX),
                          ...
                          root, unreachable in a build; do not recreate it, see P1 in Planning Docs/todo.md)
...
### 1.1 REPOSITORY-ROOT DOCUMENTS — four files at root, everything else in `Planning Docs/` (NEW 2026-08-24)

The repo root carries **exactly four** markdown files, and that is now a rule rather than an accident:

| File | What it is |
|---|---|
| `CLAUDE.md` | Coding guidelines + the standing project rules. The one file every session must obey. |
| `Claude_Project.md` | THIS file — the codebase context an agent reads to orient. |
| `Claude_TODO.md` | The living work file: thread board, open work, Bob's queue, the testing queue, change log. |
| `README.md` | Repo convention, effectively empty. Not a planning document. |

**Everything else — every plan file, pass record, courier/handoff memo and analysis — lives in
`Planning Docs/`.** That includes `Planning Docs/Claude_AI_TODO.md` (the AI track) and
`Planning Docs/Claude_TODO_Archive.md` (DONE records + the pre-2026-08-20 change log), which are working
documents like any other despite the `Claude_` prefix.

⚠ **PATHS INSIDE `Planning Docs/` ARE WRITTEN AS BARE FILENAMES AND THAT IS CORRECT** — those files
reference each other from inside the same folder (`todo_profiles.md`, not `Planning Docs/todo_profiles.md`).
Only the four root files spell the folder out. When a doc in `Planning Docs/` names one of the four root
files it means the copy at the repo root; there is no second copy.

⚠ **WHY IT EXISTS (Bob, 2026-08-24):** the root had grown to 25 markdown files, and `Claude_TODO.md` was
carrying the detail of every open thread on top of its own job. Splitting a pass's DETAIL into its own file
and leaving `Claude_TODO.md` holding only the pointer is the pattern this project already used for
`todo_icm.md` / `todo_prestige.md` / `todo_adrange.md`; the directory makes it the default rather than the
exception. **A new pass gets a new file in `Planning Docs/` plus a pointer row in `Claude_TODO.md` — never
a new root file.**

✅ **`CLAUDE.md`'s Standard Workflow was amended in step (2026-08-24, Bob's blanket "eliminate confusion"
permission):** items 1 and 7 now say "the pass's plan file in `Planning Docs/`" instead of "todo.md", so no
agent can re-create a root `todo.md` that collides with the real one (content-pipeline plan, Phase 2 paused,
now at `Planning Docs/todo.md`).

**Current focus file: `Planning Docs/Implementing Combat.md`** — the audio / combat-animation / air-ops /
combat-constants pass opened 2026-08-24. It is the singular-focus plan for that work; `Claude_TODO.md`
carries only its thread-board row and its change-log lines.
...
⚠ **§13 FORMATION-QUALITY traits are the exception to "a trait describes a platform" — they describe a FORMATION,** so under closed-bay doctrine they belong ONLY on a profile a template names in its DEPLOYED bay. Put one on a profile that exists to ride a Mobile/Embarked bay (an APC under infantry, a truck under a towed gun) and the same formation is priced twice, once per stance. The layer runs both ways: `COMBINED_ARMS_TF` ×1.15 / `AIR_CAVALRY` ×1.21 / `NATO_FIRST_LINE` ×1.10 / `FIRE_DIRECTION_NET` ×1.05 price a formation UP, `SECOND_LINE_FORMATION` ×0.9 (China only, 2026-09-02) prices one DOWN. Formation quality is not CREW quality — that stays `ExperienceLevel` on the template, and the two multiply.
⚠ **`WeaponProfileDB` also holds four COMMODITY BLUEPRINTS** (`LightTowedArtilleryDef` / `HeavyTowedArtilleryDef` / `TowedAaaDef` / `TransportTruckDef`, added RE-3a): one authored stat line feeding every nation's copy of a piece of kit that is genuinely identical. Each takes `params WeaponTrait[]` so a national trait rides on top without forking the line. A towed howitzer is the same gun for anyone — nationality changes the picture, the census, the availability turn and now the formation ICM, never the ballistics. `CommodityProfileTests` enforces that; the Mujahideen irregular variants are deliberately NOT folded in.
...
> **⚠ SUPERSEDED IN MECHANISM 2026-08-08 — P1+P2 of the profile-slot rebuild (`Planning Docs/todo_profiles.md`,
...
**GameIconRenderer:** Ground/air layers. ⚠ **FACING IS A ROTATION, NOT A SPRITE VARIANT (top-down icon pass, 2026-08-29).** Every profile has ONE sprite; `ApplyIconFacing` turns it 60° per facing off a canonical WEST heading (`RotationForFacing`, offset by the single calibration constant `ICON_HEADING_OFFSET_DEGREES`). The old per-direction art, the firing variants, `NormalizeDirection`, `ShouldFlipSprite` and the easterly `flipX` mirror are all DELETED — an east-facing unit is now the same sprite turned 180°, not a mirror image. ⚠ **It rotates `unitIcon` ONLY, never the prefab root:** the prefab carries six renderers, so turning the root would spin the HP readout, nationality flag, deployment chevron and stacking badge along with the vehicle. ⚠ **Bases never rotate** (`unit.IsBase` — HQ/DEPOT/AIRB get a meaningless side-derived `Facing` from the constructor). The pure decision is `internal static IconRotationDegrees(facing, isBase)`, extracted so `IconIntegrityTests` can drive it headlessly. `RefreshIconFacing(unitId)` still re-resolves the SPRITE — deployment position selects the bay, and a move can change both — then rotates; movement steps and Shift+click rotation call it. **Helo motion flipbook (2026-07-22):** `Prefab_CombatUnitIcon.StartMotionAnimation`/`StopMotionAnimation` cycle the 6 `<unit>_FrameN` atlas frames at a serialized fps (default 40) while the icon tween-moves — `AnimateIconStep` starts it, `SnapIcon` stops it (rests on Frame0); detection is sprite-name-based, so embarked air-mobile riding helo art animates too and non-animated icons no-op. Sprite resolution from RegimentProfile, directional flipping. Stacking: air/ground same hex → dominant at 1.0 opacity, recessive at 0.6. Toggle via EventManager. Movement animation: `AnimateIconStep(unitId, to, duration, onComplete)` (LeanTween via UnitMoveAnimator) + `SnapIcon` (cancel tween + hard-place) — driven per-hex by `MovementController.ExecuteMovement`. ⚠ Carries the tilde (~) debug enemy-reveal cheat (rendering-only; REMOVE BEFORE SHIPPING — tracked in Claude_TODO Cleanup).
...
**Volume.** An `AudioSettings` object holds Master/Music/Ambient/SFX/Briefing volumes, per-channel mutes and `MuteAll`; effective = master × channel, mute-gated. ⚠ Persisted to `Application.persistentDataPath/audio_settings.json` — a **THIRD write location**, distinct from the `Documents/My Games/Hammer and Sickle/` saves+logs pair in §1. ✅ `SaveSettings` routes through `JsonPolicy.Settings` since 2026-08-24 (the third named policy — player-written flat tree, lenient read); the CLAUDE.md item 10 cleanup is closed.
...
Covered by `AudioPolicyTests`. ⚠ Both are POLICY only — enforcement arrives with the Phase 2 facade, whose API shape is designed to make the gate unforgettable (an ungated `Play` for UI/turn/weather, a source-taking overload for anything a unit causes). See `Planning Docs/todo_audio.md`.
...
**Supply (ONE NUMBER PER UNIT — SUP-1, 2026-08-24):** every unit's pool is `DaysSupply`; caps: ground unit 5 (`MaxDaysSupplyUnit`) · airbase 30 (`MaxDaysSupplyAirbase`) · depot by size (`MaxStockpileBySize` 30/50/80/110) · **fixed-wing 0** (no own supply — the launching airbase pays; `CanMove` supply gate keys on `Max > 0`). Generation = %-of-own-cap per turn: Minimal 5% → Industrial 40%. Sorties: SORTIE_LAUNCH_COST=1, SORTIE_SHOT_COST=0.5, AIRBASE_LAUNCH_FLOOR=5, AIR_SUPPLY_LOAD=5. ⚠ `StockpileInDays` is DELETED — do not rebuild a second store (§15.1.2a).

**Persistence:** SAVE_VERSION=9 (4→5 profile-slot rebuild 2026-08-08; 5→6 over-water grace 2026-08-11; 6→7 prestige/scoring 2026-08-17; 7→8 C7 fractional gate 2026-08-20; 8→9 SUP-1 supply unification 2026-08-24 — `stockpileInDays` leaves the CombatUnit shape; the version ladder history lives at the constant in `GameData.cs`). `MINIMUM_SUPPORTED_SAVE_VERSION` tracks it pre-1.0, so older saves are refused, not migrated. Bumps still queued: AI2 (AIPerceptionState) and P5 (loss ledger), each taking its own. ⚠ `SaveLoad.SaveAsync`/`LoadAsync` still have NO callers — save/load is not wired to any UI, which is why bumps have been free.

**Prestige:** per-PROFILE `cost = PrestigeTierCost + PrestigeTypeCost` (2026-06-18 price-to-potential pass): tier Gen1(0)/Gen2(60)/Gen3(120)/Gen4(180); type INF(50)·TRK(20)·TANK(65)·IFV(55)·APC(40)·RCN(45)·ART(90)·SPA(130)·ROC(175)·BMS/AWACS(450, apex)·AAA(70)·SPAAA(135)·SAM(145)·SPSAM(175)·HEL(130)·HELT(90)·TRN(150)·FGT(150)·ATT(135)·BMB(240)·RCNA(200). Explicit exceptions live in GameData's "Prestige Exceptions" region (PRESTIGE_CRUISE_BOMBER 400). ⚠ V19 kill-prestige reads the ACTIVE profile's cost — no whole-unit price exists yet (P4 requisition).
...
| ICM | clamp 0.1–10.0, default 1.0, keep aggregate ≲1.6 (product of the profile's ICM traits; NATO apex M1 = 1.53) |
