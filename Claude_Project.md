...
_Last reconciled against the codebase: 2026-08-11 (post-hoc spotting §12.4.4a — the mover's passive spotting applies at move settlement, making §6.9 ambush reachable; §6.9.9 eligibility now enforced at the trigger. See §3.5b and §3.7. P3 movement-medium work and P1+P2 are in §3.2b)._
...
⚠ **REBUILD AFTER A FRESH CLONE — these two outputs are NOT in git.** `Assets/Resources/Chunked/TerrainArray_MiddleEast.asset` is 288 MB, over GitHub's 100 MB hard per-file limit, and `TestArray_RGB.asset` is 8 MB; both are `.gitignore`d (see the rationale block there) while the source PNGs they are built from ARE tracked. A clone therefore has no terrain array until the tools are re-run — `BindTerrainArray` logs a `Resources.Load failed` error naming the expected path when that happens — and the rebuilt asset carries a NEW GUID, so any serialized reference to it must be re-pointed.
...
**Halts are enum-keyed (`ApplyMovementHalt`, P3 2026-08-10), because the three kinds spend DIFFERENT resources** — `ZoneOfControl` keeps enough MP for a combat or intel action, `GroundAmbush` zeroes everything, and `FlightEvasion` zeroes **movement points and the move action and nothing else**. ⚠ The narrow flight rule is the ratified one (2026-08-04) and it fails INVISIBLY if widened — a flight that also lost its combat action just looks like a flight that was ambushed — so it is `internal` and pinned by `MovementTests`. It is written as "zero exactly what means this move is over" so it survives Bob's action/movement cost rebalance; a rule leaning on any particular cost would quietly change meaning under it.

**AMBUSH AGAINST A FLIGHT — WHO IS SUBJECT, BY MEDIUM (§6.9; ratified 2026-08-04, scoped 2026-08-10):** ground → the full ambush, combat and all · **HELO → the ambush TRIGGERS, the combat does NOT** · **FIXED-WING → nothing at all.**

⚠ **The helo/fixed-wing split is the load-bearing part and it follows from what each one IS.** A helo-borne regiment is **a special kind of GROUND unit** — it remains on the map, so ground troops can catch it, and it may NOT share a hex with a ground unit. A fixed-wing asset only ever *traverses* the map on its way to the air ops box: it does not spot ground units in transit (§3.7), ground troops cannot touch it, and it **CAN temporarily occupy a ground unit's hex**. Air defence reaches it only through the separate air-ambush path, which is exactly how an unspotted SAM reveals itself by firing.

For the helo case the SAME `CheckGroundAmbush` detection runs — gating it on `!isAir` is what used to make a lift immune. Only the consequence differs: no `RaiseAmbushTriggered` (that event resolves the damage), the ambusher IS revealed at **Level1** via `SpottingService.RevealAmbusherToContact` (§12.4.8 — it never fired, so not the §12.4.9.1 Level4 case), a `ReportFlightHalted` dispatch, and the `UnitMoveBlocked` sound. The flight stops on the hex it entered, adjacent to the ambusher — which makes deliberate reconnaissance-by-overflight possible, priced at the whole move plus being stranded next to an enemy. ⚠ **ZoC never stops a flight; ambush is the single mechanism.** ⚠ Vocabulary: code and comments say halted/stopped — "aborted" is narrative framing allowed only in player-facing dispatch text.
...
⚠ **§12.3.7a — A FIXED-WING AIRCRAFT IN TRANSIT DOES NOT LOOK AT THE GROUND (Bob, 2026-08-10).** `SpottingRangeAgainst` returns 0 against a GROUND target when the spotter is flying fixed-wing. Fixed-wing assets only cross the map to reach the air ops box; they neither see ground units nor are seen by them. The traffic that DOES flow is the reverse — an unspotted air-defence unit fires on them and reveals ITSELF (`CheckAirAmbush`). ⚠ **Keyed on the MEDIUM, not the classification**, because a paratroop regiment riding an An-12 is classification `AB` and would otherwise keep spotting at range 2 from inside the cargo hold. ⚠ **RECONA and AWACS are EXEMPT** — ratified look-down platforms whose 8-hex ground reach other systems are built on (§11.11.3 derives the recon mission's search area from it; §12.3.9 calls exploiting the AWACS look-down a deliberate player risk). ⚠ Helicopters are unaffected: a helo-borne unit is a special kind of ground unit and sees the ground normally. **DESIGN-DOC AMENDMENT OWED: §12.3.7 becomes 0 / 4** for FGT/ATT/BMB/WW/TRN. Because the rule sits at the single §12.3.10 comparison, the sweep, per-hex transit checks, decay floors and the AI mirror all inherit it together. `RevealAmbusherToContact(ambusher)` is the one PUBLIC reveal (P3 2026-08-10) — raises to Level1 for the flight-evasion halt, where the ambush triggers but no combat runs to reveal anyone (§3.5b); the ground path needs no equivalent because its combat sets both participants to Level4 on the way through.
